...
class mainViewModel {
  constructor() {
    this.logfile = null;
    this.worker = null;
    this.blobUrlForWorker = null;
    this.timeRangeSize = 24;
    this.timeRangeObj = {
  ...
  getSearchWordTitle() {
    var title = "";
    var length = this.searchObj.searchWords.length;
    for (var n = 0; n < length; n++) {
      title += this.searchObj.searchWords[n];
      if (n != length - 1)
        title += ",";
    }
    var maxLegth = 20;
    if (title.length > maxLegth)
      title = title.slice(0, maxLegth) + "..";
    return title;
  }
}
...
function createBlobWorker() {
  ...
  mainModel.blobUrlForWorker = URL.createObjectURL(blob);
  return new Worker(mainModel.blobUrlForWorker); // classic worker
}
...
function startWorker() {
  ...
  // 로컬용
  mainModel.worker = createBlobWorker();
  // 서버용
  //worker = createFileWorker();

  mainModel.worker.onmessage = function (e) {
    var d = e.data || {};
    if (d.status === "progress" || d.status === "done") {
      if (typeof d.loaded === "number"){
        prog.value = d.loaded;
        mainModel.totalByte = d.loaded;
      }
      const icon = d.status === "progress" ? "?" : "✅";
      const statusText = document.querySelector('#status .status-text');
      if (statusText) {
        statusText.textContent =
          icon + " " + (d.status.toUpperCase()) + " | BYTES: " + (d.loaded || 0).toLocaleString() +
      ...
      if (d.status === "done") {
        ...
        for (var i = mainModel.timeRangeSize; i > 0; i--) {
          mainModel.timeRangeSize = i;
          //console.log("timeRangeSize =",timeRangeSize);
          timeRangeMsArray = Array(mainModel.timeRangeSize).fill(0);
        ...
        var timeRangeCountArray = Array(mainModel.timeRangeSize).fill(0);
        var timeRangeCountMax_ = [0];
        var timeRangeEndByteArray = Array(mainModel.timeRangeSize).fill(0);
        ...
        mainModel.timeRangeObj.timeRangeCountArray = timeRangeCountArray;
        mainModel.timeRangeObj.xRangeArray = xRangeArray;
        mainModel.timeRangeObj.xRangeSizeArray = xRangeSizeArray;
        mainModel.timeRangeObj.timeRangeEndByteArray = timeRangeEndByteArray;

        mainModel.searchObj.searchLineStartByteArray = d.searchLineStartByteArray;
        mainModel.searchObj.searchLineEndByteArray = d.searchLineEndByteArray;

        mainModel.searchObj.searchLineArray = d.searchLineArray;
        mainModel.searchObj.searchLineTypeIndexArray = d.searchLineTypeIndexArray;
        mainModel.searchObj.searchTypeSize = d.searchTypeSize;
        mainModel.searchObj.searchTypeArray = d.searchTypeArray;
        mainModel.searchObj.searchTypeCountArray = d.searchTypeCountArray;

        console.log("searchObj.searchLineStartByteArray = ", mainModel.searchObj.searchLineStartByteArray);
        console.log("searchObj.searchLineEndByteArray = ", mainModel.searchObj.searchLineEndByteArray);
        console.log("timeRangeObj.timeRangeEndByteArray = ", mainModel.timeRangeObj.timeRangeEndByteArray);

        console.log("d.callbackMethod = ", d.callbackMethod);
        if (d.callbackMethod.length > 0) {
          window[d.callbackMethod]();
        }
      }
...
function makeXrangeArray(timeStartMs, timeLastMs, timeRangeMsArray, xRangeArray) {
  ...
  let arr = Array.from({ length: mainModel.timeRangeSize }, (_, i) => parseInt((timeLastMs - timeStartMs) / mainModel.timeRangeSize * (i + 1)));
  arr[mainModel.timeRangeSize - 1] = timeLastMs;
...
function cleanupWorker() {
  if (mainModel.worker) {
    mainModel.worker.terminate();
    mainModel.worker = null;
  }
  if (mainModel.blobUrlForWorker) {
    URL.revokeObjectURL(mainModel.blobUrlForWorker);
    mainModel.blobUrlForWorker = null;
  }
}

// --- 파일 스트리밍 → 워커로 chunk 전송 (Transferable로 복사 없이 이동) ---
async function startByFile(file, searchWord, startByte, endByte, type, callbackMethod) {
  if (!mainModel.worker) startWorker();

  mainModel.searchObj.searchWords = searchWord;
  ...
  mainModel.worker.postMessage({ cmd: "start", searchWord: searchWord, type: type, callbackMethod: callbackMethod });
  for (; ;) {
    var r = await reader.read();
    if (r.done) break;
    var value = r.value; // Uint8Array
    // ArrayBuffer를 transfer list로 넘겨 복사 방지
    mainModel.worker.postMessage({ cmd: "chunk", chunk: value.buffer }, [value.buffer]);
  }
  mainModel.worker.postMessage({ cmd: "end" });
}
...
function doSearch() {
  ...
  if (mainModel.logfile == null)
    ...
    startByFile(mainModel.logfile, [searchWord], 0, 0, "search", "");
  }
}
    ...
    function loadPage(page) {
      ...
      getLogsInTimeRange(selectTimeRangeStartIndex, selectTimeRangeEndIndex, page).then(logs => {
        ...
        const content = `
          ...
          <div class="wrap" id="logRowList" style="max-height: 580px; overflow-y: auto; width: 100%"></div>
        `;
...
async function getLogsInTimeRange(startIdx, endIdx, page = 1) {
  const logs = [];
  const startByte = startIdx === 0 ? 0 : mainModel.timeRangeObj.timeRangeEndByteArray[startIdx - 1];
  const endByte = mainModel.timeRangeObj.timeRangeEndByteArray[endIdx];
  ...
  const rangeIndices = [];
  for (let i = 0; i < mainModel.searchObj.searchLineStartByteArray.length; i++) {
    const logStart = mainModel.searchObj.searchLineStartByteArray[i];
    const logEnd = mainModel.searchObj.searchLineEndByteArray[i];
...
function readLogSegment(start, end) {
  return new Promise((resolve, reject) => {
    ...
    if (end > mainModel.logfile.size) {
      console.warn(`End byte ${end} exceeds file size ${mainModel.logfile.size}, adjusting...`);
      end = mainModel.logfile.size;
    }
    
    const slice = mainModel.logfile.slice(start, end);
...
function updateDetailContent() {
  ...
  const endByte = Math.min(mainModel.logfile.size, currentLog.endByte + 10240); // 뒤 10KB
...
**💡 권장사항**
- 구체적이고 실행 가능한 조치사항(예시 소스코드 작성 포함)
...
async function doDetailAIAnalyze(logText, aiModel, aiLength) {
  ...
  try {
    ...
    resultModal.setContent(resultContainer);
	    ...
	    // 3. AI 응답을 박스 형태로 재조립할 컨테이너 생성
	    const aiBoxesContainer = document.createElement('div');
	    aiBoxesContainer.className = 'ai-analysis-container'; // CSS에서 정의한 메인 컨테이너

	    // 4. AI가 보낸 원본 마크다운 텍스트
	    const rawMarkdown = (typeof result.content === 'string' && result.content) || '분석 결과가 없습니다.';

	    // 5. [핵심] 마크다운을 HTML로 변환 후, 임시 DOM에 삽입
	    const tempDiv = document.createElement('div');
	    // marked.js가 필요합니다! (이전 답변 참고)
	    tempDiv.innerHTML = marked.parse(rawMarkdown);

	    let currentBox = null;
	    
	    // 프롬프트에서 요청한 섹션 제목들
	    const headings = ['📊 분석 요약', '🔍 상세 분석', '💡 권장사항', '⚠️ 주의사항'];

	    // 6. [핵심] 변환된 HTML 노드를 순회하며 박스 재조립
	    Array.from(tempDiv.childNodes).forEach(node => {
	        ...
	        if (isHeading && node.tagName === 'P') { // 제목 노드(<p><strong>...</strong></p>)
	            // 새 박스 생성
	            currentBox = document.createElement('div');
	            currentBox.className = 'ai-result-box';
	            
	            // 박스 헤더 생성
	            const header = document.createElement('div');
	            header.className = 'ai-result-header';
	            header.innerHTML = node.innerHTML; // <p> 안의 <strong>...</strong> 를 그대로 복사
	            ...
	            // (첫 번째 섹션 제목이 나오기 전의 컨텐츠 - 만약 있다면)
	            aiBoxesContainer.appendChild(node.cloneNode(true));
	        }
	    });

	    // 7. 재조립된 박스 컨테이너를 모달 컨텐츠에 추가
	    modalContentContainer.appendChild(aiBoxesContainer);

	    // 8. 모달에 최종 컨텐츠 설정
	    resultModal.setContent(modalContentContainer);
	...
	*/
	const modelButtons = modalContentContainer.querySelectorAll('.btn-model');
	modelButtons.forEach(btn => {
	  btn.addEventListener('click', () => {
	    const selectedModel = btn.getAttribute('data-model');
	    resultModal.close();
...
function doSplit() {
  ...
  const fileSizeMB = (mainModel.logfile.size / (1024 * 1024)).toFixed(2);
    ...
    <div style="padding: 20px; font-family: 'SF Mono', Monaco, monospace; color: #00ffff;">
      <div style="margin-bottom: 20px;">
        ...
        <div style="background: rgba(0, 255, 255, 0.1); padding: 12px; border: 1px solid rgba(0, 255, 255, 0.3); border-radius: 4px;">
          <div>파일명: <span style="color: #ff69b4;">${mainModel.logfile.name}</span></div>
  ...
  modal.setFooter([
    {
      ...
      onClick: async (m) => {
        ...
        // 파일 분할 실행
        await splitFile(mainModel.logfile, splitSizeMB);
