...
var workerSource = `
  ...
  var type = "search";  // search,  analyze 
  ...
  // 어레이 버퍼 수
  var searchlineMax = 10000*300;
  var searchtypeMax = 10000*10;
  var byteEndsMax = 10000*10;
  ...
  // 서치분석
  var searchLineTypeIndexArray;  // 검색 라인 타입 인덱스 어레이 
  var searchTypeIndex = 0;
  var searchTypeArray;        // search type 이 들어간다.
  var searchTypeCountArray;  // search type 카운트가 들어간다.
  ...
  function init() {
    ...
    isInException = false;
	// 서치분석
	searchLineTypeIndexArray = new Uint32Array(searchlineMax); 
    searchTypeIndex = 0;
    searchTypeArray = new Array(searchtypeMax); 
    searchTypeCountArray = new Uint32Array(searchtypeMax); 
  }
  ...
  function parseLine(line, _searchword,lineStartByte,lineEndByte) {
	...
	if (type != "analyze"){
		...
		for(var i=0;i<searchWord.length;i++){
		  if (line.indexOf(searchWord[i]) >= 0){
			isSearch = true;
			break;
		  }
		}
		...
		if (isSearch) {
		  if (type == "analyze" && searchLineIndex < searchtypeMax){
			var isDuplicate = false;
			for(n=0;n<searchTypeIndex;n++){
			  if( line == searchTypeArray[n]){
				isDuplicate = true;
				searchTypeCountArray[n] = searchTypeCountArray[n] + 1;
				searchLineTypeIndexArray[searchLineIndex] = n;
				break;
			  }
			}
			if(!isDuplicate){
			  searchTypeArray[searchTypeIndex] = line;
			  searchLineTypeIndexArray[searchLineIndex] = searchTypeIndex;
			  searchTypeCountArray[searchTypeIndex] = 1;
			  searchTypeIndex++;
			}
		  }
		  searchLineArray[searchLineIndex] = timeCur;
  ...
  function reportProgress(status) {
      ...
      postMessage({
        ...
        preview: textBuffer.length > 200 ? textBuffer.slice(-100) : textBuffer,    // 미리보기: 마지막 부분만
        // done 용
        searchLineArray: status == "done" ? searchLineArray : null,
        timeStart: status == "done" ? timeStart : 0,
        timeLast: status == "done" ? timeLast : 0,
        searchLineStartByteArray: status == "done" ? searchLineStartByteArray : null,
        searchLineEndByteArray: status == "done" ? searchLineEndByteArray : null,
        searchLineTypeIndexArray: status == "done" ? searchLineTypeIndexArray : null,
        searchTypeSize: status == "done" ? searchTypeIndex : 0,
        searchTypeArray: status == "done" ? searchTypeArray : null,
        searchTypeCountArray: status == "done" ? searchTypeCountArray : null,
        callbackMethod: status == "done" ? callbackMethod : "",
      });
  }
  ...
  function flushAndFinish() {
    ...
    if (isInException && exceptionBuffer) {
      if (searchWord[0] === 'java.lang.Exception' || searchWord[0] === 'Exception' || exceptionBuffer.indexOf(searchWord[0]) >= 0) {
        searchLineArray[searchLineIndex] = timeCur;
        searchLineStartByteArray[searchLineIndex] = exceptionStartByte;
        searchLineEndByteArray[searchLineIndex] = exceptionEndByte;
        searchLineIndex++;
